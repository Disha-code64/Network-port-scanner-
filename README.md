...
# ---------------------------
class PortScanner:
    def init(self, target, start_port, end_port, timeout=0.5, max_workers=500):
        self.target = target
...
class ScannerGUI(tk.Tk):
    ...
    def start_scan(self):
        ...
        if not (0 <= start_port <= 65535 and 0 <= end_port <= 65535 and start_port <= end_port):
            messagebox.showerror("Input Error", "Port range must be within 0â€“65535 and start â‰¤ end.")
            return
